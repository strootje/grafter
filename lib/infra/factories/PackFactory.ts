...
export class PackFactory {
	...
	public static Create(folder: string): Pack {
		...
		switch (packType) {
			case 'assets': return new ResourcePack(packType, packName, folder, opts);
			case 'data': return new DataPack(packType, packName, folder, opts);
