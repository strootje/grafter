...
import { MergedFile } from '../domain/files';
...
export abstract class Pack extends EventEmitter {
	...
	constructor(
		...
		this.watcher.on('add', (filename: string) => {
			this.HandleFileAdded(filename);
			this.TryValidate();
		});

		this.watcher.on('unlink', (filename: string) => {
	...
	protected abstract ReceiveFileAdd(filename: string): void;
	protected abstract ReceiveFileDelete(filename: string): void;
	...
	private HandleFileAdded(filename: string): void {
		this.ReceiveFileAdd(filename);
	}

	private HandleFileDeleted(filename: string): void {
		this.ReceiveFileDelete(filename);
	}
